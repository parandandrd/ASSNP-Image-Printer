...
outfile="/mnt/d/sticky-processed.bmp"
#
...
convert "$infile" -scale "${neww}x${newh}" -threshold 50% -colors 2 -flip "BMP3:${outfile}"

# Send the $outfile processed image via the $ippfile instructions to the printer at $printerip
ipptool -f "$outfile" "ipp://${printerip}/" "$ippfile"
